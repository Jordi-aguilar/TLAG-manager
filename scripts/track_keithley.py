...
class Window(QWidget):
    ...
    def __init__(self, sampleinterval=0.5, timewindow=10., size=(600,350)):
        ...
        self.files = [
            self.common_path + "resistance/resistance_" + self.current_file,
        ]
        ...
        labels = [
            {"bottom" : "Time (seconds)", "right" : "Resistance (Ohms)"}
        ]

        self.rows = [(0, 1)]
        self.legend = ["Resistance"]
    ...
    def update_text(self):
        ...
        text_time = "<span style='color: gregrayen'>Time={:0.1f} seconds, </span>".format(self.xs[1][self.current_indices[0]-1] - self.t0)
        span_close = "<\span>"
        self.label.setText(span_style_size + text_time + text_temperature + text_power+ text_pressure + span_close) #(data1[index], data2[index])
    ...
    def updatecurve(self, i): # file, curve, currend_idx
        ...
        if i == 0 and curr_idx % 5 == 0:
            print(curr_file, curr_idx, curr_rows)
            print(curr_idx, len_new_data, x, y)
