...
class Window(QWidget):
    ...
    def __init__(self, sampleinterval=0.5, timewindow=10., size=(600,350)):
        ...
        self.new_file = False
        # self.common_path = "/homelocal/opbl11/Documents/TLAG/TLAG-tool_SAFA/experiment_manager/results/"
        self.common_path = r"C:\Users\User\Documents\TLAG-tool_SAFA\experiment_manager\results"

        # Define number of curves to updates
        self.num_curves = 3

        self.files = [
            os.path.join(self.common_path, "temperature", "temperature_" + self.current_file),
            os.path.join(self.common_path, "temperature", "temperature_" + self.current_file),
            os.path.join(self.common_path, "pressure", "pressure_" + self.current_file)
        ]

        self.file_log = os.path.join(self.common_path, "logs", "log_" + self.current_file.split('.')[0] + '.log')
        ...
        #Labels reles
        self.label_rotary = QLabel("ROTARY + MOVE")
        ...
        for i, plot in enumerate(self.plots):
            layout.addWidget(plot, i+1, 0, 1, 2)


        layout.addWidget(self.label_rotary, i+2, 0, 1, 1)
        layout.addWidget(self.label_turbo, i+2, 1, 1, 1)


        layout.addWidget(self.combobox_file, i+3, 0, 1, 2)
        layout.addWidget(self.button_update_combobox, i+4, 0, 1, 2)
    ...
    def update_combobox(self):
        files_dir = os.listdir(os.path.join(self.common_path, "temperature"))
    ...
    def update_relay_move(self):
        ...
        if move_position is not None:
            self.label_rotary.setText("ROTARY + MOVE ({}%)".format(move_position))
